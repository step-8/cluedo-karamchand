...
const { findPossibleMoves } = require('../src/utils/findPossibleMoves.js');
...
describe('findPossibleMoves', () => {
  it('Should give possible positions for single move', () => {
    const actual = findPossibleMoves(gameDetails, 1, [0, 7]);
    const expected = [[1, 7]];
  ...
  it('Should give possible room positions for given moves', () => {
    const actual1 = findPossibleMoves(gameDetails, 1, [4, 7]);
    const actual2 = findPossibleMoves(gameDetails, 2, [4, 7]);
  ...
  it('Should not give room if entry point is invalid', () => {
    const actual = findPossibleMoves(gameDetails, 1, [7, 19]);
  ...
  it('Should give possible moves from inside room', () => {
    const actual1 = findPossibleMoves(gameDetails, 1, [6, 19]);
    const actual2 = findPossibleMoves(gameDetails, 2, [6, 19]);
